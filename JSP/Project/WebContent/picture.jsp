...
<div class="container">
	...
	<table class="table">
  	...
  	<c:forEach items="${list}" var="picture" >
    	<tr>
      		<th scope="row">${picture.bId}</th>
      		<td>${picture.bName}</td>
      		<td>
      			<c:forEach begin="1" end="${picture.bIndent}">-</c:forEach>
				<a href="content_view.do?bId=${picture.bId}">${picture.bTitle}</a>
      		</td>
      		<td>${picture.bDate}</td>
      		<td>${picture.bHit}</td>
    	</tr>
			...
			<td colspan="5" >
				<nav aria-label="..." class="nav justify-content-center">
	  				<ul class="pagination" id="nav">	  
	  					<c:choose>
							<c:when test="${(page.curPage - 1) < 1}">
	    						<li class="page-item disabled" >
	      							<a class="page-link" href="picture.do?page=1" tabindex="-1">처음</a>
	    						</li>
	    					</c:when>
	    					<c:otherwise>
	    						<li class="page-item">
	      							<a class="page-link" href="picture.do?page=1">처음</a>
	   							</li>
	   						</c:otherwise>
	  					</c:choose>
	  						...
	  						<c:choose>
	  							<c:when test="${page.curPage == fEach}">
	    							<li class="page-item active">
	      								<a class="page-link" href="picture.do?page=${fEach}">${fEach} <span class="sr-only">(current)</span></a>
	    							</li>
	    						</c:when>
	    						<c:otherwise>
	    							<li class="page-item"><a class="page-link" href="picture.do?page=${fEach}">${fEach}</a></li>
	    						</c:otherwise>
	    					</c:choose>
	  					</c:forEach>
	  					<c:choose>
	    				...
	    				<c:otherwise>
	      					<li class="page-item"><a class="page-link" href="picture.do?page=${page.totalPage}">끝</a></li><br>
	  					</c:otherwise>
	  					</c:choose>
					</ul>
				</nav>
